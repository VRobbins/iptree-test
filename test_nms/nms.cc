...
int32_t nms::add_ipv4_plain(const char *a_buf, uint32_t a_buf_len)
{
        ...
        if(!arr)
        {
                arr = new ipv4_set_t[33];
        }
        arr[32].insert(l_in.s_addr);
        return WAFLZ_STATUS_OK;
}
//: ----------------------------------------------------------------------------
//: \details: TODO
...
int32_t nms::add_ipv4_cidr(const char *a_buf, uint32_t a_buf_len)
{
        ...
        if(!arr)
        {
                arr = new ipv4_set_t[32];
        }
        arr[l_bits].insert(l_masked_addr);
        return WAFLZ_STATUS_OK;
}
//: ----------------------------------------------------------------------------
//: \details: TODO
...
/*int32_t nms::add_ipv6_plain(const char *a_buf, uint32_t a_buf_len)
{
        ...
        if(!arr6)
        {
                arr6 = new ipv6_set_t[129];
        }
        arr6[128].insert(l_in6);
        return WAFLZ_STATUS_OK;
}
//: ----------------------------------------------------------------------------
//: \details: TODO
...
int32_t nms::add_ipv6_cidr(const char *a_buf, uint32_t a_buf_len)
{
        ...
        arr6[l_bits].insert(l_masked_addr);
        return WAFLZ_STATUS_OK;
}
//: ----------------------------------------------------------------------------
//: \details: TODO
...
int32_t nms::contains_ipv4(bool &ao_match, const char *a_buf, uint32_t a_buf_len)
{
        ...
        for(int i = 32; i>-1; --i)
        {
                const uint32_t l_nm = (i == 0) ? 0 : htonl(~((1 << (i)) - 1));
                if(arr[i].find(l_nm & l_in.s_addr)!= arr[i].end())
                {
                        ao_match = true;
                        return WAFLZ_STATUS_OK;
                }
